...
def search_number(handle, handle_with_number)

  if !handle.scan(/\d+/).empty?
    handle_with_number << handle.scan(/\d+/) 
  end

  return handle_with_number
  return handle

end

def search_aude(handle, handle_with_aude)
  
  if handle.match(/[Aa][Uu][Dd][Ee]/)
    handle_with_aude << handle
  end

  return handle_with_aude
  return handle

end

def search_at_maj(handle, handle_with_at_maj)
    if handle.match(/@[A-Z]/)
    handle_with_at_maj << handle
  end

  return handle_with_at_maj
  return handle

end

def search_maj(handle, handle_with_maj)
    if handle.match(/[A-Z]/)
    handle_with_maj << handle
  end

  return handle_with_maj
  return handle

end

def search_underscore(handle, handle_with_underscore)
    if handle.match(/[_]/)
    handle_with_underscore << handle
  end

  return handle_with_underscore
  return handle

end


def search_journalist(journalists,handle_with_number,handle_with_aude,handle_with_at_maj,handle_with_maj,handle_with_underscore)
  journalists.each do |handle|
    search_number(handle, handle_with_number)
    search_aude(handle, handle_with_aude)
    search_at_maj(handle, handle_with_at_maj)
    search_maj(handle, handle_with_maj)
    search_underscore(handle, handle_with_underscore)
  end
end

def sort_journalists(journalists)
  sort_value = gets.chomp  
  check_if_field_is_empty(sort_value,journalists)
end

def check_if_field_is_empty(sort_value,journalists)
    if !sort_value.empty?
      yes_or_no(sort_value,journalists)
    else
      # print "Merci de répondre \"y\" ou \"n\""
      sort_journalists(journalists,sort_value)
    end
end


def yes_or_no(sort_value,journalists)
      if sort_value == "y"
